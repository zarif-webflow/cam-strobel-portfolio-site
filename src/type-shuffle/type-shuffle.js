...
export class TypeShuffle {
  ...
  effects = {
    fx1: (props) => this.fx1(props),
    fx2: (props) => this.fx2(props),
    fx3: (props) => this.fx3(props),
    fx4: (props) => this.fx4(props),
    fx5: (props) => this.fx5(props),
    fx6: (props) => this.fx6(props),
  };
  totalChars = 0;
  ...
  fx3(props) {
    const { maxCellIters, freq1, freq2 } = props;
    const MAX_CELL_ITERATIONS = Number.isInteger(maxCellIters) ? maxCellIters : 5;
    let finished = 0;
    this.clearCells();
    ...
    const loop = (line, cell, iteration = 0) => {
      ...
      ++iteration;
      if (iteration < MAX_CELL_ITERATIONS) {
        setTimeout(() => loop(line, cell, iteration), Number.isInteger(freq1) ? freq1 : 80);
      }
    };

    for (const line of this.lines) {
      for (const cell of line.cells) {
        setTimeout(() => loop(line, cell), randomNumber(0, Number.isInteger(freq2) ? freq2 : 300));
      }
    }
  }
  fx4() {
   ...
   * @param {Object} props - for additional control.
   * @param {number} props.maxCellIters
   * @param {number} props.freq1
   * @param {number} props.freq2
   */
  trigger(effect = 'fx1', props = {}) {
    if (!(effect in this.effects) || this.isAnimating) return;
    this.isAnimating = true;
    this.effects[effect](props);
  }
}
